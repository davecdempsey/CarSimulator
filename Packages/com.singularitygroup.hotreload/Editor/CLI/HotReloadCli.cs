...
namespace SingularityGroup.HotReload.Editor.Cli {
    ...
    public static class HotReloadCli {
        ...
        public static Task StartAsync() {
            return StartAsync(exposeServerToNetwork: HotReloadPrefs.ExposeServerToLocalNetwork, allAssetChanges: HotReloadPrefs.AllAssetChanges);
        }
        
        internal static async Task StartAsync(bool exposeServerToNetwork, bool allAssetChanges) {
            await Prepare().ConfigureAwait(false);
            await ThreadUtility.SwitchToThreadPool();
            StartArgs args;
            if(TryGetStartArgs(UnityHelper.DataPath, exposeServerToNetwork, allAssetChanges, out args)) {
        ...
        internal static async Task RestartAsync(bool exposeServerToNetwork, bool allAssetChanges) {
            await Prepare().ConfigureAwait(false);
            await ThreadUtility.SwitchToThreadPool();
...
#pragma warning restore CS0649
        ...
        static bool TryGetStartArgs(string dataPath, bool exposeServerToNetwork, bool allAssetChanges, out StartArgs args) {
            string serverDir;
